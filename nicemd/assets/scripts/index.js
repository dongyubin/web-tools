...
function getAlink() {
    ...
    for (let i = 0; i < ad.length - 1; i++) {
        ...
        let code = "<section class='qrcode-box' style='margin-top: 1rem; border-radius: 0.5rem; background-color: rgb(249, 250, 251); color: rgb(55, 65, 81); display: flex; justify-content: space-between; border: 1px solid rgb(239, 239, 239); box-shadow: rgb(204, 204, 204) 2px 4px 7px; margin-bottom: 20px;'><section class='qrcode-text' style='display: flex; flex-direction: column; justify-content: space-around; word-break: break-all; padding: 1rem; font-size: 0.875rem; line-height: 1.25rem; '><span style='line-height: inherit; margin-top: 0px;'>长按识别二维码查看原文</span><span>" + ad_href + "</span><span>标题：" + ad_text + "</span></section><section class='qrcode-img' style='width: 124px; height: 124px; flex-shrink: 0;'><img alt='" + ad_href + "' src='" + ad_qrcode_href + "' class='medium-zoom-image' style='max-width: 100%; border-radius: 6px; display: block; object-fit: contain; margin: auto; box-shadow: none;'></section></section>"
